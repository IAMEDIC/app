...
def _prepare_frame(ds: pydicom.Dataset, frame: np.ndarray) -> np.ndarray:
    ...
    img = _to_uint8(frame)
    # Invert for MONOCHROME1 if needed (black/white inverted)
    try:
...
def process_dicom(file_bytes: bytes, filename: str) -> Tuple[bytes, str, str]:
    ...
    pixel_array = ds.pixel_array  # may be shape (frames, rows, cols) or (rows, cols[, frames])

    if number_of_frames > 1:
        # Multi-frame -> video
        # Normalize to shape (frames, rows, cols[, channels])
        frames: List[np.ndarray] = []
        arr = pixel_array
        # Handle (rows, cols, frames) grayscale
        if arr.ndim == 3 and arr.shape[2] == number_of_frames and getattr(ds, 'SamplesPerPixel', 1) == 1:
            arr = np.transpose(arr, (2, 0, 1))  # -> (frames, rows, cols)
        # Handle 4D with channels last: (rows, cols, channels, frames)
        if arr.ndim == 4 and arr.shape[3] == number_of_frames:
            arr = np.transpose(arr, (3, 0, 1, 2))  # -> (frames, rows, cols, channels)
        if arr.ndim == 3:
            for i in range(arr.shape[0]):
                frames.append(_prepare_frame(ds, arr[i]))
        elif arr.ndim == 4:
            for i in range(arr.shape[0]):
                frames.append(_prepare_frame(ds, arr[i]))
        else:
