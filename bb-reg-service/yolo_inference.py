...
) -> tuple[cv2.typing.MatLike, float, int, int]:
...
def preprocess_images(
    images: NDArray[np.float32] | list[cv2.typing.MatLike],
...
class YOLOONNXInference:
    ...
    def __call__(
        ...
        raw_output = self.session.run(None, {input_name: inp})[0]  # [B, 6, 8400]
        ...
        for _, (batch_dets, meta) in enumerate(zip(detections_per_batch, metas)):
            if len(batch_dets) > 0:
