...
/**
 * Simple pie chart component using MUI and CSS for file count display
 */
export const FileCountChart: React.FC<StorageChartProps> = ({
  activeFiles,
  softDeletedFiles,
  title,
  totalLabel,
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            ...
            <Typography variant="body2">
              {activeFiles.label}: {activeFiles.value} ({activeFiles.percentage.toFixed(1)}%)
            </Typography>
          </Box>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            ...
            />
            <Typography variant="body2">
              {softDeletedFiles.label}: {softDeletedFiles.value} ({softDeletedFiles.percentage.toFixed(1)}%)
            </Typography>
          </Box>
        </Box>
      </CardContent>
